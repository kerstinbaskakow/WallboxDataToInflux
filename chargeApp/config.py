...
class Config:
    ...
    ACTIVE_PHASES_DET_INFLUX = "activePhasesDetected"
    ACTIVE_PHASES_DET_INFLUX_QUERY = 'SELECT * FROM {} ORDER BY time DESC LIMIT 1'.format(ACTIVE_PHASES_DET_INFLUX)
    CHARGE_STATE_INFLUX = "ChargingState" 
    CHARGE_STATE_INFLUX_QUERY = 'SELECT * FROM {} ORDER BY time DESC LIMIT 1'.format(CHARGE_STATE_INFLUX)
    CHARGE_POWER_INFLUX = "ChargePower_W" 
    CHARGE_POWER_INFLUX_QUERY = 'SELECT * FROM {} ORDER BY time DESC LIMIT {}'.format(CHARGE_POWER_INFLUX,MEAN_AMOUNT)
    PV_POWER_INFLUX = "modbus.0.holdingRegisters.40067_PV_Leistung" 
    PV_POWER_INFLUX_QUERY = 'SELECT * FROM "{}" ORDER BY time DESC LIMIT {}'.format(PV_POWER_INFLUX,MEAN_AMOUNT)
    BATTERY_POWER_INFLUX = "modbus.0.holdingRegisters.40069_Batterie_Leistung"
    BATTERY_POWER_INFLUX_QUERY = 'SELECT * FROM "{}" ORDER BY time DESC LIMIT {}'.format(BATTERY_POWER_INFLUX,MEAN_AMOUNT)
    HOME_POWER_INFLUX = "modbus.0.holdingRegisters.40071_Hausverbrauch_Leistung"
    HOME_POWER_INFLUX_QUERY = 'SELECT * FROM "{}" ORDER BY time DESC LIMIT {}'.format(HOME_POWER_INFLUX,MEAN_AMOUNT)